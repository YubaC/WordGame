...
function setlist(list_input, id_input, order_input) {
    ...
    for (i = 0; i < list_input.length; i++) {
        ...
        liObj.innerHTML =
            '<a name="' +
            id_input[id_input.indexOf(order_input[i])] +
            '" href="javascript:void(0);" onclick="getmap(this.name)">' +
            list_input[id_input.indexOf(order_input[i])] +
            "</a>";
        // 把js新建的li放到ul下
...
function edit(list_input, id_input, order_input) {
    ...
    for (i = 0; i < list_input.length; i++) {
        ...
        liObj.innerHTML +=
            '<a name="' +
            id_input[id_input.indexOf(order_input[i])] +
            '" href="javascript:void(0);" onclick="rename(this.name)">&nbsp;&nbsp;&nbsp;重命名</a>';
        liObj.innerHTML +=
            '<a name="' +
            id_input[id_input.indexOf(order_input[i])] +
            '" href="javascript:void(0);" onclick="delete_map(this.name)">&nbsp;&nbsp;&nbsp;删除</a>';
        // 把js新建的li放到ul下
        maplist.appendChild(liObj);
    }
}
...
function save() {
    ...
    settings[4] = [
        ...
        peace.checked,
        unmatched.checked,
        Number(document.getElementById("boom_power").value),
    ];
...
function read(map_id_input) {
    settings_input = decodeURIComponent(
        window.atob(getCookie(map_id_input + "_settings"))
    ).split(";");
    ...
    for (ch = 0; ch < settings_input[3].length - 1; ch++) {
        document.getElementById(settings_input[3][ch]).checked = eval(
            settings_input[4][ch]
        );
    }
    ...
    document.getElementById("in").value = decodeURIComponent(
        window.atob(getCookie(map_id_input))
    );
    ...
    for (g = 0; g < map_input.length - 3; g++) {
        //除去最后三行，剩下的赋值给map数组
        map[g] = map_input[g];
    }
